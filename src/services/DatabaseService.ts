...
export class DatabaseService {
  ...
  private readonly TABLE_NAME = 'brand'; // Default table name, will try this first
  private actualTableName: string = 'brand'; // Will be updated after determining which table exists
  ...
  async initDB(): Promise<SQLite.SQLiteDatabase> {
    ...
    try {
      ...
      try {
        console.log(
          'Trying to open database from assets with createFromLocation: 1',
        );
        ...
        dbOpenError = error1;
        console.warn(
          'Failed to open with createFromLocation: 1. Error:',
          error1,
        );
        ...
        try {
          console.log(
            'Trying to open database with exact path "~output.sqlite"',
          );
          this.db = await SQLite.openDatabase({
            name: 'output.sqlite',
            location: 'default',
          ...
          console.log(
            'Database opened successfully with createFromLocation: ~output.sqlite',
          );
        } catch (error2) {
          ...
          try {
            ...
            throw new Error(
              'Could not open or create database after multiple attempts',
            );
          }
        }
      }
    ...
    try {
      ...
      try {
        ...
        if (tableCheck[0].rows.length === 0) {
          console.log(
            `Table '${this.TABLE_NAME}' doesn't exist yet, will be created if needed`,
          );
      ...
      try {
        ...
        if (indexCheck[0].rows.length === 0) {
          ...
          await this.db.executeSql(
            `CREATE INDEX IF NOT EXISTS idx_records_name ON ${this.TABLE_NAME}(name)`,
          );
    ...
    try {
      ...
      for (const name of possibleTableNames) {
        if (tableOptions.includes(name)) {
          ...
          try {
            const columnsResult = await this.db.executeSql(
  ...
  private async _createSampleData(): Promise<void> {
    ...
    try {
      ...
      const countCheck = await this.db.executeSql(
        `SELECT COUNT(*) as count FROM ${this.TABLE_NAME}`,
      );
      ...
      if (recordCount === 0) {
        ...
        console.log(
          `Table already contains ${recordCount} records, skipping sample data creation`,
        );
      }
    } catch (error) {
      if (this.db) {
  ...
  async getTotalRecordCount(): Promise<number> {
    ...
    try {
      ...
      try {
        ...
        if (tableCheck[0].rows.length === 0) {
          console.warn(
            `Table ${this.TABLE_NAME} doesn't exist, returning count of 0`,
          );
          return 0;
        }
      } catch (tableCheckError) {
        console.error(
          'Error checking table existence during count:',
          tableCheckError,
        );
        return 0;
      }
  ...
  async getRecords(params: PaginationParams): Promise<QueryResult> {
    ...
    try {
      ...
      console.log(
        `Fetching records for page ${page}, pageSize ${pageSize}, offset ${offset}`,
      );
      ...
      try {
        ...
        console.log(
          `Query executed successfully, returned ${queryResults[0].rows.length} rows`,
        );
        ...
        try {
          ...
          try {
            ...
            console.log(
              'Simple query succeeded, manually filtering results...',
            );

            // Since we couldn't use LIMIT/OFFSET in SQL, do it in code
            const clampedOffset = Math.min(
              offset,
              simpleResults[0].rows.length,
            );
            const endIdx = Math.min(
              clampedOffset + pageSize,
              simpleResults[0].rows.length,
            );
            ...
            console.log(
              `Manually paginated results: ${queryResults[0].rows.length} rows`,
            );
          } catch (simpleError) {
      ...
      console.log(
        `Processed ${records.length} records for current page ${page}`,
      );
  ...
  async searchRecordsByName(
    ...
    try {
      ...
      console.log(
        `Searching for: "${searchText}" with page=${page}, pageSize=${pageSize}`,
      );
      ...
      try {
        ...
        if (tableCheck[0].rows.length === 0) {
          console.warn(
            `Table ${this.TABLE_NAME} doesn't exist for search operation`,
          );
        ...
        console.error(
          'Error checking table existence during search:',
          tableCheckError,
        );
      }
      ...
      try {
        ...
        console.log(
          `Search query executed successfully, returned ${results[0].rows.length} rows`,
        );
      } catch (directQueryError) {
        ...
        try {
          ...
          try {
            ...
            const simpleResults = await db.executeSql(simpleQuery, []);
            console.log(
              'Simple search succeeded, manually filtering results...',
            );
      ...
      try {
        console.log(
          `Executing direct count query: SELECT COUNT(*) as count FROM ${this.actualTableName} WHERE name LIKE '${searchParam}'`,
        );
        const countResults = await db.executeSql(
          `SELECT COUNT(*) as count FROM ${this.actualTableName} WHERE name LIKE '${searchParam}'`,
          [],
        );
      ...
      const totalPages = Math.ceil(totalCount / pageSize);
      console.log(
        `Search found ${totalCount} total matches across ${totalPages} pages`,
      );
